...
def setup_paths(environment, dataset_name):
    if environment == 'cluster':
        if dataset_name == "Chime6":
            return '/export/data2/nbaier/espnet/egs2/chime7_task1/asr1/dataset/ChiME6/audio/train'
        else:
            return "/project/data_asr/dipco/Dipco"  
    else:
        if dataset_name == "Chime6":
            return "/home/niklas/Downloads/Datasets/CHIME6/CHiME6_eval/CHiME6/audio/train"
        else:
            return "/project/data_asr/dipco/Dipco"  
...
def string_parsing(dataframe):
    ...
    dataframe.drop(columns=['endframe', 'session_id', 'gender', 'nativeness','mother_tongue','audio','start','end','endframe','duration','frames', 'ref'], inplace=True)
    dataframe.reset_index(drop=True, inplace=True)
    return dataframe 

expanded_df = string_parsing(df)
#num_speakers = expanded_df['speaker_id_int'].unique()
#print(num_speakers)
...
# create split of the training dataset 
...
# adding a classification head to get the speaker (class)

class Seq2SeqWithClassificationHead(torch.nn.Module):
    def __init__(self, model, num_labels):
        super(Seq2SeqWithClassificationHead, self).__init__()
        self.trained_seq2seq = model
        self.classfication_head = nn.Linear(model.config.hidden_size, num_labels)
    def forward(self, input_ids, attention_mask=None, decoder_input_ids=None, labels = None):
        outputs = self.trained_seq2seq(input_ids)
        hidden_state = outputs[0]
        pooled_output = hidden_state.mean(dim=1)
        logits = self.classfication_head(pooled_output)
        return logits 
    
       
num_labels = num_speakers
model = Seq2SeqWithClassificationHead(model=model, num_labels=num_labels)
