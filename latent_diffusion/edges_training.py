...
# Improved training function with PyTorch built-ins
def train_rectified_flow(model, train_dataloader, val_dataloader=None, num_epochs=30, lr=1e-4, scheduler_type='onecycle'):
...
def main():
    ...
    train_dataloader = DataLoader(
        train_dataset, 
        batch_size=4,  # Reduced batch size for 256x256 images
        shuffle=True, 
        num_workers=2,
        pin_memory=True,
    ...
    model = RectifiedFlowUNet(
        ...
        base_channels=128  # Good for 256x256 images
    ).to(device)
    ...
    plt.title('Training Loss - Edges to Shoes')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    torch.save(model.state_dict(), 'rectified_flow_edges_to_shoes.pth')
    torch.save(ema_model.state_dict(), 'rectified_flow_edges_to_shoes_ema.pth')
    print("Models saved: 'rectified_flow_edges_to_shoes.pth' and 'rectified_flow_edges_to_shoes_ema.pth'")
